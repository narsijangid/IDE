...
function olkil_payu_enrich_subscription( array $sub ) {
	...
	if ( ! $current ) {
		...
		$had_paid            = ! empty( $sub['plan'] ) && 'dazzlone' !== sanitize_key( (string) $sub['plan'] );
		$out['is_expired']   = $had_paid || ! empty( $sub['is_expired'] ) || ( isset( $sub['status'] ) && 'expired' === $sub['status'] );
	...
	foreach ( $ents as $ent ) {
		...
		$held[] = array(
			...
			'tokens_left'        => $e_left,
			'tokens_left_label'  => olkil_payu_format_tokens( $e_left ),
			'tokens_total_label' => olkil_payu_format_tokens( (int) ( $ent['tokens_total'] ?? 0 ) ),
	...
	return array(
		...
		'spendable_left_label'=> olkil_payu_format_tokens( $spendable ),
