...
export class BrowserTestService {
  ...
  async launch(headed = true, forceNew = false): Promise<BrowserActionResult> {
    ...
    if (!forceNew && this.launchInFlight) {
      return this.launchInFlight;
    }

    const run = this.launchInternal(headed, forceNew);
    this.launchInFlight = run;
    try {
      return await run;
    } finally {
      if (this.launchInFlight === run) {
        this.launchInFlight = null;
      }
    }
  }

  private async launchInternal(headed = true, forceNew = false): Promise<BrowserActionResult> {
  ...
  /** Best-effort window bounds — never block first paint. */
  private async positionWindowFast(): Promise<void> {
  ...
  async liveTest(request: LiveTestRequest): Promise<LiveTestResult> {
    ...
    // App boot + browser launch in parallel
    const [, launch] = await Promise.all([startServer(), launchPromise]);
    ...
    // Fast URL discovery — short probes, no long serial waits before navigate.
    if (!request.url && commandId) {
      const deadline = Date.now() + Math.min(request.readyTimeoutMs ?? 12_000, 12_000);
      while (Date.now() < deadline) {
        ...
        for (const candidate of detect.suggestedUrls) {
          const probe = await waitForHttp(candidate, 280);
          if (probe.ok) {
            target = candidate;
            urls = [candidate];
            found = true;
        ...
        if (urls.length && (await waitForHttp(urls[0], 280)).ok) {
          target = urls[0];
          break;
        }
        await sleep(160);
    ...
    const ready = await waitForHttp(target, request.readyTimeoutMs ?? 8_000);
    if (!ready.ok) {
