# [experimental] @olkil/llms

`@olkil/llms` is the model and provider layer for the Cline SDK. It gives
...
- `@olkil/llms/runtime` for declarative config and runtime registry creation
- `@olkil/llms/providers` for handler creation and provider settings/types
- `@olkil/llms/models` for model catalogs and query helpers
- `@olkil/llms` root exports for the gateway registry and shared llm contracts

## Installation

```bash
npm install @olkil/llms zod
```

## Quick Start

```ts
import { createHandler } from "@olkil/llms";
...
```ts
import { createLlmsRuntime, defineLlmsConfig } from "@olkil/llms/runtime";
```

### Providers

Use `@olkil/llms/providers` for:
...
`@olkil/llms` and `@olkil/shared`.

### Models

Use `@olkil/llms/models` when you need generated provider/model metadata for
...
- `@olkil/llms`: runtime-focused convenience entrypoint
- `@olkil/llms/node`: explicit Node/runtime entrypoint
- `@olkil/llms/browser`: browser-safe bundle
- `@olkil/llms/runtime`: focused runtime entrypoint
- `@olkil/llms/models`: model catalog/query entrypoint
- `@olkil/llms/providers`: provider handler/settings entrypoint

## Related Packages

- `@olkil/agents`: agent loop and tool execution
- `@olkil/core`: stateful runtime assembly and provider settings storage
...
bun -F @olkil/llms run test:live
```
