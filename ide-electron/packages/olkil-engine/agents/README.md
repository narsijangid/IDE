# [experimental] @olkil/agents

`@olkil/agents` is the runtime-agnostic agent loop package in the Cline SDK.
...
`@olkil/agents` does not ship a full application runtime by itself.

- Default host tools like filesystem access, shell execution, or web fetching live in `@olkil/core`
- Session persistence and stateful orchestration live in `@olkil/core`
- Shared hub runtime/session transport lives in `@olkil/core` (see `@olkil/core/hub`)
- Sub-agent and team coordination primitives live in `@olkil/core`
...
```bash
npm install @olkil/agents @olkil/shared @olkil/llms
```

## Quick Start

```ts
import { Agent } from "@olkil/agents";
import type { AgentTool } from "@olkil/shared";
...
when the host already owns gateway construction (this is what `@olkil/core`
uses internally):

```ts
import { createGateway } from "@olkil/llms";
...
`@olkil/shared`. Each tool has a JSON Schema `inputSchema` and an
...
`AgentRuntimeEvent` in `@olkil/shared` for the full union.
...
subprocess-backed hooks, MCP extensions), use `@olkil/core`.
...
```ts
import type { AgentRuntimePlugin } from "@olkil/shared";
...
For multi-agent workflows, use `@olkil/core`:
...
- `@olkil/agents` — the single package entrypoint. The `package.json`
  `exports` map automatically serves a browser-safe bundle when bundlers
  resolve the `browser` condition.

## Related Packages

- `@olkil/shared`: shared types (`AgentTool`, `AgentMessage`,
  `AgentRuntimeEvent`, `AgentRuntimeHooks`, etc.)
- `@olkil/llms`: provider settings, model catalogs, and gateway/handler
  creation
- `@olkil/core`: stateful runtime assembly, storage, default tools,
