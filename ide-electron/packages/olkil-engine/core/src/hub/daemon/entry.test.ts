...
vi.mock("@olkil/shared", async (importOriginal) => {
	const actual = await importOriginal<typeof import("@olkil/shared")>();
	return {
		...actual,
		initVcr: mockInitVcr,
...
describe("hub daemon entry", () => {
	...
	it("ignores abort-family unhandled rejections instead of shutting down", async () => {
		...
		const { AgentRuntimeAbortError } = await import("@olkil/agents");
