# [experimental] @olkil/core

`@olkil/core` is the stateful orchestration layer of the Cline SDK. It
...
- host-facing Node helpers through `@olkil/core`

## Installation

```bash
npm install @olkil/core
```

## Entry Points

- `@olkil/core`: core contracts, shared utilities, and Node/server helpers for building hosts and runtimes

## Typical Usage

Most host apps should start with `@olkil/core`.

```ts
import { ClineCore } from "@olkil/core";
...
Use `@olkil/core` for host-facing runtime assembly:
...
`@olkil/core` owns the built-in host tools and executors:
...
- `SqliteTeamStore`
- SQLite-backed local session stores and artifacts through `@olkil/core`

## Related Packages

- `@olkil/agents`: stateless agent loop and tool primitives
- `@olkil/llms`: provider/model configuration and handlers
