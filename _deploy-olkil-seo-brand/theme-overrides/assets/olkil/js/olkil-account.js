...
(function () {
  ...
  function wireUi(auth) {
    var authed = document.getElementById('olkil-account-authed');
    var toggle = document.getElementById('olkil-account-toggle');
    var closeTimer = null;
    ...
    function scheduleClose() {
      ...
      closeTimer = setTimeout(function () {
        authed.classList.remove('is-open');
        if (toggle) toggle.setAttribute('aria-expanded', 'false');
      }, 180);
    }

    if (authed) {
      authed.addEventListener('mouseenter', openMenu);
      authed.addEventListener('mouseleave', scheduleClose);
      authed.addEventListener('focusin', openMenu);
      authed.addEventListener('focusout', function (e) {
        if (!authed.contains(e.relatedTarget)) scheduleClose();
      });
    }
    ...
    if (toggle) {
      toggle.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (authed.classList.contains('is-open')) {
          authed.classList.remove('is-open');
